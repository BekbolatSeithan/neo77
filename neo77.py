...
st.markdown("""
    <style>
    /* Глубокий черный фон для самого приложения */
    ...
    /* 3D Фон растянут на весь экран и находится сзади */
    [data-testid="stHtml"] {
    ...
    /* Сообщения в виде стекла поверх ядра */
    .stChatMessage { 
...
# --- 4. СТАТИЧНЫЙ 3D КОД (РЕНДЕРИТСЯ ТОЛЬКО ОДИН РАЗ) ---
# Мы больше не передаем Python-переменные внутрь строки. Этот код заморожен.
STATIC_HTML = """
<!DOCTYPE html>
<html>
<head>
...
for(let i=0; i<count; i++) {
    ...
    // УВЕЛИЧЕНО В 1.5 РАЗА (было 1.6, стало 2.4)
    let r = 2.4; 
...
// СЕКРЕТНЫЙ ТРЮК: JS-код сам проверяет, думает ли бот, читая сайт
setInterval(() => {
    try {
        if (window.parent && window.parent.document) {
            const flag = window.parent.document.getElementById('neo77-thinking-flag');
            isThinking = !!flag; // Если флаг есть, значит бот думает (красный цвет)
        }
    } catch(e) { }
}, 200);
...
function animate() {
    ...
    // ПЛАВНЫЙ ПЕРЕХОД ЦВЕТА И СКОРОСТИ
...
# --- 5. ИНТЕРФЕЙС ЧАТА ---
st.markdown("<h2 style='text-align: center; color: white; letter-spacing: 2px; font-family: Courier;'>NEO77 TERMINAL</h2>", unsafe_allow_html=True)
...
if prompt := st.chat_input("NEO77 жүйесіне сұрақ қойыңыз..."):
    st.session_state.messages.append({"role": "user", "content": prompt})
    st.session_state.is_thinking = True 
    st.rerun()

if st.session_state.is_thinking:
    # СОЗДАЕМ НЕВИДИМЫЙ ФЛАГ, ЧТОБЫ ЯДРО СТАЛО КРАСНЫМ
    st.markdown("<div id='neo77-thinking-flag' style='display:none;'></div>", unsafe_allow_html=True)
    st.markdown("<p style='color: #ff0055; text-align: center; font-family: Courier; font-weight: bold;'>[ РАСПОЗНАВАНИЕ И АНАЛИЗ... ]</p>", unsafe_allow_html=True)
    ...
    # Даем голосу договорить (около 8.5 символов в секунду)
    audio_duration = max(3.0, len(full_response) / 8.5)
    time.sleep(audio_duration) 
    
    # Обновляем страницу, скрытый флаг пропадает, ядро плавно синеет
    st.rerun()